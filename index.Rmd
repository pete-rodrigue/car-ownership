...
p1 <-
  ...
  xlab("") +
  ylab("") +
  ggtitle("For example: About 1.4% of households own four or more vehicles") +
...
ggplot(data = trt_d_co,
  ...
  geom_text(x = 50, y = 80, label = lm_eqn(trt_d_co, x = "med_hh_inc", "nv_hh"), parse = TRUE) +
  ylab("# vehicles / # households") +
  xlab("Median household income (thousands of $)") +
  theme_minimal() +
  ggtitle("Areas with higher household incomes have more vehicles") +
...
* https://lindseyresearch.com/wp-content/uploads/2021/12/NHTSA-2021-0053-1575-Exhibit-41-McCarthy-1996.pdf 
* https://media.rff.org/documents/WP_23-33.pdf
* https://www.sciencedirect.com/science/article/pii/S0965856400000185
...
trt_d_co <- 
  ...
  dplyr::left_join(y = trt %>% sf::st_drop_geometry(), by = "TRACT") %>%
    mutate(
          nv_hh = round(nv / num_hh*100),
          nv_po = round(nv / pop_over_18*100),
          n_hh_w_atleast_1_v_divided_by_n_hh = round(n_hh_w_atleast_1_v / num_hh, 2)
          ) %>%
  mutate(
    nv_hh = ifelse(nv_hh > 100, 100, nv_hh),
    nv_po = ifelse(nv_po > 100, 100, nv_po)
...
ggplot(data = trt_d_co,
       aes(x = med_hh_inc/1000, y = n_hh_w_atleast_1_v_divided_by_n_hh)) +
  geom_point() +
  geom_smooth(method = "lm") +
  # geom_text(x = 50, y = .60,
  #           label = lm_eqn(trt_d_co, x = "med_hh_inc", "n_hh_w_atleast_1_v_divided_by_n_hh"), parse = TRUE) +
  ylab("# HH with at least one vehicle, as a share of the # HH") +
  xlab("Median household income (thousands of $)") +
  theme_minimal() +
  ggtitle("In areas with higher household incomes more households own vehicles") +
  labs(caption = "Note: tract income is top-coded at $250,000") +
  theme(plot.caption = element_text(hjust = 0))
```
...
ggplot() +
  ...
  labs(title = "More than 43,000 vehicles are registered\nwithin a 10-minute walk of a metro station") +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
    legend.position = "right",
...
metro_blob <- st_union(mto_buffer, by_feature = F)
co_inside_blob <- co[st_within(co, metro_blob, sparse = FALSE), ]
co_outside_blob <- co[!st_within(co, metro_blob, sparse = FALSE), ]
round(mean(co_inside_blob$`Count of Vehicles`, na.rm=T), 2)
round(mean(co_outside_blob$`Count of Vehicles`, na.rm=T), 2)

quantile(co_inside_blob$`Count of Vehicles`, probs = c(.5, .75, .8, .9, .95), na.rm=T)
quantile(co_outside_blob$`Count of Vehicles`, probs = c(.5, .75, .8, .9, .95), na.rm=T)
```
